...
class PV3Fit(nn.Module):
    def __init__(self,):
        super(PV3Fit, self).__init__()
        self.pv3 = PointTransformerV3()
        self.shs_mlp1 = nn.Conv1d(190, 128, 1)
        self.shs_mlp2 = nn.Conv1d(128, 64, 1)
        self.shs_mlp_dc = nn.Conv1d(64, 3, 1)
        self.shs_mlp_rest = nn.Conv1d(64, 9, 1)
        ...
        self.scale_mlp = nn.Conv1d(148, 128, 1)
        self.scale_mlp1 = nn.Conv1d(128+20+64, 64, 1)
        self.scale_mlp2 = nn.Conv1d(64+20+64, 3, 1)
        nn.init.kaiming_uniform_(self.scale_mlp2.weight)
        nn.init.constant_(self.scale_mlp2.bias, -5)

        self.rotation_mlp = nn.Conv1d(148, 128 , 1)
        self.rotation_mlp1 = nn.Conv1d(128+20+64, 64 , 1)
        self.rotation_mlp2 = nn.Conv1d(64+20+64, 4 , 1)
        ...
        self.opacity_mlp = nn.Conv1d(64, 1, 1)
    ...
    def forward(self, point_dict, dist, dgcnninput):
        output = self.pv3(point_dict)
        pred_feature = output['feat']
        # print(pred_feature.shape)
        pointcloud = point_dict['feat']
        
        
        shs = self.leaky_relu(self.shs_mlp1(torch.cat([pred_feature, pointcloud],1).unsqueeze(0).permute(0,2,1)))
        shs = self.leaky_relu(self.shs_mlp2(shs))

        
        shs_dc = -2*F.sigmoid(self.shs_mlp_dc(shs))
        shs_rest = 0.3*F.tanh(self.shs_mlp_rest(shs))
        # shs = shs_dc
        shs = torch.cat([shs_dc,shs_rest],1)
        dgcnnfeature = self.convdgcnn(dgcnninput)
        dgcnnfeature = dgcnnfeature.max(dim=-1, keepdim=False)[0]
        
        scaleinput = torch.cat([pred_feature.unsqueeze(0).permute(0,2,1),dist.permute(0,2,1),dgcnnfeature],1)
        scale1 = self.leaky_relu(self.scale_mlp(scaleinput))
        scaleinput2 = torch.cat([scale1,dist.permute(0,2,1),dgcnnfeature],1)
        scale1 = self.leaky_relu(self.scale_mlp1(scaleinput2))
        scaleinput3 = torch.cat([scale1,dist.permute(0,2,1),dgcnnfeature],1)

        # scale = torch.clamp(torch.exp(self.scale_mlp2(scaleinput3)), max=0.005)
        scale = torch.nn.Softplus()(self.scale_mlp2(scaleinput3))
        rotationinput = torch.cat([pred_feature.unsqueeze(0).permute(0,2,1),dist.permute(0,2,1),dgcnnfeature],1)
        rotation1 = self.leaky_relu(self.rotation_mlp(rotationinput))
        rotationinput2 = torch.cat([rotation1,dist.permute(0,2,1),dgcnnfeature],1)
        rotation2 = self.leaky_relu(self.rotation_mlp1(rotationinput2))
        rotationinput3 = torch.cat([rotation2,dist.permute(0,2,1),dgcnnfeature],1)
  
        rotation = torch.nn.functional.normalize(self.rotation_mlp2(rotationinput3))
        opacity = torch.sigmoid(self.opacity_mlp(pred_feature.unsqueeze(0).permute(0,2,1)))
        # print(scale.shape)
        ptsnums = shs.shape[2]
        return shs.permute(0,2,1).reshape(1, ptsnums, 4, 3), scale.permute(0,2,1), rotation.permute(0,2,1), opacity.permute(0,2,1)
    
class PV3Align(nn.Module):
    def __init__(self,):
        super(PV3Align, self).__init__()
        self.pv3 = PointTransformerV3(in_channels=144)
        self.shs_mlp1 = nn.Conv1d(208, 128, 1)
        self.shs_mlp2 = nn.Conv1d(128, 128, 1)
        self.shs_mlp3 = nn.Conv1d(128, 64, 1)
        self.shs_mlp_dc = nn.Conv1d(64, 32, 1)
        self.shs_mlp_dc2 = nn.Conv1d(32, 3, 1)
        self.shs_mlp_rest = nn.Conv1d(64, 64, 1)
        self.shs_mlp_rest2 = nn.Conv1d(64, 9, 1)
    ...
    def forward(self, point_dict, bs):
        output = self.pv3(point_dict)
        pred_feature = output['feat']
        # print(pred_feature.shape)
        pointcloud = point_dict['feat']
        
        tmpinput = torch.cat([pred_feature, pointcloud],1).reshape(bs, 90000, 208).permute(0,2,1)
        # print(tmpinput.shape)
        shs = self.leaky_relu(self.shs_mlp1(tmpinput))
        shs = self.leaky_relu(self.shs_mlp2(shs))
        shs = self.leaky_relu(self.shs_mlp3(shs))

        shs_dc = self.leaky_relu(self.shs_mlp_dc(shs))
        shs_dc = -2*F.sigmoid(self.shs_mlp_dc2(shs_dc))
        shs_rest = self.leaky_relu(self.shs_mlp_rest(shs))
        shs_rest = 0.3*F.tanh(self.shs_mlp_rest2(shs_rest))
