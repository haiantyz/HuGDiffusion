...
def training(dataset, opt, pipe, lp, args, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    GSModel.load_state_dict(checkpoint)
   

    
    GSModel.train()

    optimizer = optim.Adam(GSModel.parameters(), lr=0.0003)
    ...
    for epoch in range(0,1300):
        angles = range(360)
        angles = list(angles)
        random.shuffle(angles)
        radomidx = epoch%360
        if radomidx ==0:
            with open('/home/tang_21/Project/gaussian-splatting/testimage/psnr7.txt','a') as f:    
                f.write("----------\n")
        radomangle = angles[radomidx]
        for pcs, colors, shsgt, scalegt, rotationgt, opacitygt, persons in train_data_loader:
            ...
            for i in range(bs):
                ...
                render_pkg = render(pcs[i,:,:3], shs_pred[i], scalegt[i], rotationgt[i], opacitygt[i],  world_view_transform, FovX, full_proj_transform, camera_center, pipe, bg)
                ...
                loss += (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image*mask , imagegt*mask ))
                
                
            if epoch%100==0:
                torch.save(GSModel.state_dict(), "GSunify"+str(epoch)+".pt")
